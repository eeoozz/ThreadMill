...
public:
	...
	template <typename T, typename = typename
		std::enable_if<sizeof(typename T::value_type) == sizeof(uint8_t)>::type>
	int recv(T& message, IPv4& ipaddr, uint32_t quat[]) const
	{
		...
		memcpy(&quat[0],std::begin(buffer) +  0,sizeof(int32_t));
		memcpy(&quat[1],std::begin(buffer) +  4,sizeof(int32_t));
		memcpy(&quat[2],std::begin(buffer) +  8,sizeof(int32_t));
		memcpy(&quat[3],std::begin(buffer) +  12,sizeof(int32_t));
		return ret;
	}
